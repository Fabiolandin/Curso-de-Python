...
class Conta:
    ...
    def sacar(self: object, valor: float) -> None:
        if valor > 0 and self.saldo_total >= valor:
            if self.saldo > valor:
                self.saldo = self.saldo - valor
                self.saldo_total = self._calculca_saldo_total
            else:
            ...
            print('Erro ao sacar. Tente novamente')
    ...
    def transferir(self: object, destino:object, valor: float) -> None:
        if valor > 0 and self.saldo_total >= valor:
            if self.saldo >= valor:
                ...
                self.saldo_total = self._calcula_saldo_total
                destino.saldo = destino.saldo + valor
                destino.saldo_total = destino._calcula_salto_total
            print('Transferencia realizada com sucesso!')
        else:
            print('Erro ao transferir. Tente novamente')
